...
export const TmdbDetailsResponseSchema = z.object({
  ...
  spoken_languages: z.array(z.object({ english_name: z.string() })).optional(),
  ...
  networks: z.array(z.object({ name: z.string(), logo_path: z.string().nullable() })).optional(),
...
export async function getTmdbInfo(
  ...
  return {
    ...
    spokenLanguages: details.spoken_languages?.map((l) => l.english_name) ?? null,
    productionCountries: details.production_countries?.map((c) => c.name) ?? null,
    ...
    networks: details.networks?.map((n) => ({
      name: n.name,
      logo: n.logo_path ? `https://image.tmdb.org/t/p/w200${n.logo_path}` : null,
    })) ?? null,
    createdBy: details.created_by?.map((c) => c.name) ?? null,
